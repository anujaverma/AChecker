...
//                    $font_weight = BasicChecks::get_p_css ( $e, "font-weight" );
	...
	//
	//        public static function checkEffectDifferenceForLinkWCAG2A($cssPropriety) {
	//
	//            global $global_e, $global_content_dom;
	//            global $showSurroundingTextExample;
	//            global $background, $foreground;
	...
	//                $parent=$e->parent();
	//
	//            if(BasicFunctions::inMulticlass($parent))
		//		    return true;
	//
	...
	//                   if($surroundingTextColor == '')
		//                       $surroundingTextColor = 'black';
	//
	//                   $textColor = BasicChecks::getForeground($e);
	//                   if($textColor == '')//standard link color: ok
	//                       return true;
	//
	...
	//
	//
	//                //check if element a has siblings which are not "a" elements
	//
	//                $prevS = $e->prev_sibling();
	//                while($prevS != null && $prevS->tag == 'a')
			//                    $prevS = $e->prev_sibling();
	//                $nextS = $e->next_sibling();
	//                while($nextS != null && $nextS->tag == 'a')
		//                    $nextS = $e->next_sibling();
	//
	//                if($prevS == null && $nextS == null)
			//                {
	//                    //no siblings
	//                    //check if parent contains simple text (free text, not in an alement)
	//                    if(strlen(trim($parent->plaintext))>0)
		//                    {
	//                        //get parent color
	//                    }
	//
	...
	//                else
		//                {
	//                    $color = BasicChecks::get_p_css($e, "color");
			//
			//                    if($prevS != null)
		//                    {
			//                        $prevSColor = BasicChecks::get_p_css($prevS, "color");
	//                    }
					//
	//                }
	//
	//                //check if parent contains text around child element a
	//                //TODO
	//                if(strlen(trim($parent->plaintext)) > strlen(trim(BasicChecks::aText($e))))
		//                {
	//                    $hasText=1;
	//
	//                    //echo("<p>".BasicChecks::getElementPlainText($parent)."</p>");
	//                }
	//
	//
	//
	...
	////                    //
	////                    if($parent->children($i)->tag!="a"){
	////                        $hasText=1;
	////                        echo("ha testo");
	////                    }
	////                }
	//                //se il contenitore contiene altro testo
	//                if($hasText){
	//                    $outerTextForeground=BasicChecks::get_p_css($parent, "color");
	//                    $innerTextForeground=BasicChecks::getForegroundA($e, $cssPropriety);
	//
	//                    //se il colore del link non è determinato, le opzioni di default del browser vanno bene
	//                    if ($innerTextForeground == "undetermined" || $innerTextForeground == "" || $innerTextForeground == null)
		//			return true;
	//                    //se il colore del testo non è determinato, setto il valore a "black"
	//                    if ($outerTextForeground == "undetermined" || $outerTextForeground == "" || $outerTextForeground == null)
		//			$outerTextForeground='black';
	//
	//                    $innerColor = BasicChecks::convert_color_to_hex ( $innerTextForeground );
			//                    $outerColor = BasicChecks::convert_color_to_hex ( $outerTextForeground );
			//                    $background = $outerColor;
			//                    $foreground = $innerColor;
			//
			//                    $res = '';
			//                    $res = BasicChecks::ContrastRatio ( strtolower ( $innerColor ), strtolower ( $outerColor ) );
			//
			//                    //se il colore è poco contrastante, controllo gli effetti
			//                    if ($res < 3) {
			//			//controllo sugli effetti
			//                        $innerTextStyle=BasicChecks::get_p_css_a($e, "font-style", $cssPropriety);
			//                        //se è applicato qualche effetto, va bene
			//                        if($innerTextStyle!="" && $innerTextStyle!=null && $innerTextStyle!="undetermined" && $innerTextStyle!="normal"){
			//                            return true;
			//                        }
			//                        $fontWeightP = BasicChecks::get_p_css_a($e, "font-weight", $cssPropriety);
			//                        $fontWeight = BasicChecks::get_p_css($e, "font-weight");
			//
			//                        if($fontWeightP != $fontWeight)
		//                        {
			//                            return true;
			//                        }
			//                        //se nessun effetto è applicato controllo le decorazioni
			//                        $innerTextDecoration=BasicChecks::get_p_css_a($e, "text-decoration", $cssPropriety);
			//                        //se ci sono decorazioni va bene, altrimenti check non soddisfatto
			//                        if($innerTextDecoration!="" && $innerTextDecoration!=null && $innerTextDecoration!="undetermined" && $innerTextDecoration!="none")
	//                            return true;
	//                        else
		//                        {
